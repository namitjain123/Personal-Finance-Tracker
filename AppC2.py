...
def update_income(amount):
    try:
        ...
        cursor.execute(
            ...
            (amount, st.session_state.current_user)
        )
        connection.commit()
    except Exception as e:
        st.error(f"Database error: {e}")
...
def add_expense(date, category, amount, description):
    try:
        ...
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?)""",
            (st.session_state.current_user, date, category, amount, description)
        )
        connection.commit()
    except Exception as e:
        st.error(f"Database error: {e}")
...
def get_user_expenses():
    try:
        ...
        return pd.DataFrame(expenses, columns=['Date', 'Category', 'Amount', 'Description'])
    except Exception as e:
        st.error(f"Database error: {e}")
        return pd.DataFrame()
    finally:
...
def calculate_remaining_budget():
    ...
    try:
        ...
        # Get income from database instead of session state
        try:
            connection = get_db_connection()
            cursor = connection.cursor()
            cursor.execute("SELECT income FROM users WHERE username = ?", (st.session_state.current_user,))
            result = cursor.fetchone()
            monthly_income = float(result[0]) if result else 0.0
        finally:
            if connection:
                connection.close()
...
if not st.session_state.user_logged_in:
    ...
    # Get current income from database
    try:
        connection = get_db_connection()
        cursor = connection.cursor()
        cursor.execute("SELECT income FROM users WHERE username = ?", (st.session_state.current_user,))
        result = cursor.fetchone()
        current_income = float(result[0]) if result else 0.0
    finally:
        if connection:
            connection.close()
    ...
    with col1:
        st.metric("Monthly Income", format_indian_currency(current_income))
    with col2:
